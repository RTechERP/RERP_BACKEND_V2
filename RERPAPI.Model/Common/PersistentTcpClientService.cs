...
namespace RERPAPI.Model.Common
{
    public class PersistentTcpClientService
    {
        ...
        {
            var data = Encoding.UTF8.GetBytes(message); return SendAsync(data, cancellationToken);
        }
        public async Task<string> SendAndReceiveStringAsync(string message, CancellationToken cancellationToken = default)
        {
            await SendStringAsync(message, cancellationToken); return await ReceiveStringAsync(4096, cancellationToken);
        }
        public async Task<byte[]> ReceiveAsync(int maxBytes = 4096, CancellationToken cancellationToken = default)
        {
            await EnsureConnectedAsync(cancellationToken); var buffer = new byte[maxBytes]; int bytesRead;
            try
            {
        ...
        {
            var data = await ReceiveAsync(maxBytes, cancellationToken); return Encoding.UTF8.GetString(data);
        }
