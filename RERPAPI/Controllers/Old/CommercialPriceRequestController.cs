    ...
    public class CommercialPriceRequestController : ControllerBase
    {
            ...
            )
        {
            try
            {
                ...
                var data1 = await SqlDapper<object>.ProcedureToListTAsync(
                    "spGetCommercialPriceRequest",
                    new
                    {
                        Keyword = keyword,
                        YearNo = yearNo,
        ...
        public async Task<IActionResult> SaveData([FromBody] List<CommercialPriceRequestImportDTO> lstDTO)
        {
         ...
         : item.IsSaleQuotedText.Trim().ToLower().Equals("sale báo", StringComparison.OrdinalIgnoreCase) ? 1
         : item.IsSaleQuotedText.Trim().ToLower().Equals("sale không báo", StringComparison.OrdinalIgnoreCase) ? 0
         : null,
