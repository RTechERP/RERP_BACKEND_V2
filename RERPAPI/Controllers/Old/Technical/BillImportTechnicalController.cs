...
namespace RERPAPI.Controllers.Old.Technical
{
	...
	public class BillImportTechnicalController : ControllerBase
	{
		...
		public IActionResult GetCheckHistoryCheck(DateTime dateStart, DateTime dateEnd, int employeeId, int employeeBorrowId, int supplierId, int wareHouseId, string? filterText, int warehouseType)
		{
			try
			{
				var dt = SQLHelper<dynamic>.ProcedureToList("spGetBillImportTechHistory",
													new string[] { "@DateStart", "@DateEnd", "@EmployeeID", "@EmployeeBorrowID", "@SupplierID", "@FilterText", "@WarehouseID", "@WarehouseType" },
													new object[] { dateStart, dateEnd, employeeId, employeeBorrowId, supplierId, filterText, wareHouseId, warehouseType });
