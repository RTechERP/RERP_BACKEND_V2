...
namespace RERPAPI.Controllers.Old
{
    ...
    public class ModulaLocationController : ControllerBase
    {
        ...
        [HttpPost("savedata")]
        public async Task<IActionResult> SaveData([FromBody] List<SerialNumberModulaLocation> serialNumberModulaLocations)
        {
            try
            {
        ...
        //        var resultLazerOff = await _tcpClient.SendAndWaitResponseAsync(_lazerOffModula, ct);

        //        var resultCall = await _tcpClient.SendAndWaitResponseAsync(_returnModula, ct);

        //        if (string.IsNullOrEmpty(resultCall) || !resultCall.Contains('|'))
        //        {
        //            return BadRequest(ApiResponseFactory.Fail(null, "Không nhận được phản hồi!"));
        //        }

        //        await _tcpClient.SendAndWaitResponseAsync(_displayClearModula, ct);
