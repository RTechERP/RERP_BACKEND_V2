    ...
    string billExportDetailIds = "")
        ...
        public IActionResult getImageSignature(int id)
        {
            try
            {
                ...
                Employee emReciver = _employeeRepo.GetAll(x => x.UserID == billExp.UserID).FirstOrDefault() ?? new Employee(); // Người giao
                Employee emDeliver = _employeeRepo.GetAll(x => x.UserID == billExp.SenderID).FirstOrDefault() ?? new Employee(); // Người nhận
