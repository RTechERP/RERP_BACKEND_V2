...
namespace RERPAPI.Controllers.Project
{
    ...
    public class ProjectPartlistPriceRequestController : ControllerBase
    {
        ...
        private POKHRepo pOKHRepo;
        private ProjectPartlistPriceRequestRepo requestRepo;
        private ProductSaleRepo productSaleRepo;
        private CurrencyRepo currencyRepo;
        private SupplierSaleRepo supplierSaleRepo;
        private ProjectSolutionRepo projectSolutionRepo;
        private EmployeeSendEmailRepo employeeSendEmailRepo;
        private ProjectPartlistPriceRequestTypeRepo projectPartlistPriceRequestTypeRepo;
        private ProjectPartlistPriceRequestNoteRepo projectPartlistPriceRequestNoteRepo;
        ...
        #endregion Lấy tất cả yêu cầu báo giá

        #region API lấy dữ liệu
                ...
                foreach (var item in request.Products)
                {
                    try
                    {
                        ...
                        if (requestModel.ID <= 0 || request.ProjectPartlistPriceRequestTypeID == 7)
                        {
                            ...
                            await _PPPRLogRepo.AddLog(requestModel.ID, $"{currentUser.FullName} đã thêm mới yêu cầu mua hàng!", "Thêm mới");
        ...
        #endregion Update Price Request Status (Từ chối / Hủy từ chối báo giá)
        ...
        public async Task<IActionResult> QuotePriceRequest(List<ProjectPartlistPriceRequest> lst)
        {
            try
            {
                ...
                foreach (var item in lst)
                {
                    ProjectPartlistPriceRequest request = requestRepo.GetSingleNoTracking(x => x.ID == item.ID);
                    ...
                    //}
                    ////hủy báo giá lại
                    //else if (oldStatus==7 && newStatus==1)
                    //{
                    //    newStatus = 6;
                    //    request.DatePriceQuote = null;
