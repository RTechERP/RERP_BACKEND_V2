    ...
    public class ProjectSolutionController : ControllerBase
    {
        ...
        public async Task<IActionResult> SaveData([FromBody] ProjectSolutionDTO request)
        {
            try
            {
                ...
                if (request.ID > 0)
                {
                    await _projectSolutionRepo.UpdateAsync((ProjectSolution)request);
                    return Ok(ApiResponseFactory.Success((ProjectSolution)request, "Cập nhật thành công"));
                }
                else
                {
                    await _projectSolutionRepo.CreateAsync((ProjectSolution)request);
                    return Ok(ApiResponseFactory.Success((ProjectSolution)request, "Tạo mới thành công"));
                }
            }
            catch (Exception ex)
            {
