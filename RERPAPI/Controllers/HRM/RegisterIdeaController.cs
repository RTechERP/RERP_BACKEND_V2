	...
	public class RegisterIdeaController : ControllerBase
	{
		...
		public IActionResult GetRegisterIdeaDetail(int id, int currentUserEmployeeId)
		{
			try
			{
				...
				List<Model.Entities.Department> de = _departmentRepo.GetAll();
				List<RegisterIdeaDetail> rgtd = _registerIdeaDetailRepo.GetAll(x => x.RegisterIdeaID == id);
				List<RegisterIdeaFile> rgtf = _registerIdeaFileRepo.GetAll(x => x.RegisterIdeaID == id);
		...
		public IActionResult GetAllRegisterIdeaType()
		{
			try
			{
				var data = _registerIdeaTypeRepo.GetAll(x => x.IsDeleted == false);
				return Ok(ApiResponseFactory.Success(data, ""));
			}
			catch (Exception ex)
			{
