    ...
    public class OfficeSupplyRequestsController : ControllerBase
    {
        ...
        public IActionResult GetdataDepartment()
        {
            try
            {
                ...
                {
                    var departmentIDs = new List<int> { 9, 10, currentUser.DepartmentID };
        ...
        public IActionResult GetOfficeSupplyRequests(string? keyword,DateTime? monthInput, int? departmentID) 
        {
            try
            {
                ...
                if (_roleConfig.UserAllsOfficeSupply?.Contains(currentUser.EmployeeID) == true)
                {
                ...
                List<List<dynamic>> result = SQLHelper<dynamic>.ProcedureToList(
                    ...
                    new object[] { month, key, effectiveEmployeeId, effectiveDepartmentId }
                );
                return Ok(new
                {
                    status = 1,
                    data = result.Count > 0 ? result[0] : new List<dynamic>()
                });
            }
            catch (Exception ex)
            {
